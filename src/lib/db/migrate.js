...
async function runVersionedMigrations(adapter) {
  ...
  const current = parseInt(await getMetaFromAdapter(adapter, "schemaVersion", "0"), 10) || 0;
  ...
  for (const m of pending) {
    await adapter.transaction(async () => {
      await m.up(adapter);
      await setMetaFromAdapter(adapter, "schemaVersion", m.version);
    });
...
async function syncSchemaFromTables(adapter) {
  ...
  for (const [tableName, def] of Object.entries(TABLES)) {
    ...
    await adapter.exec(buildCreateTableSql(tableName, def, isPostgres));
...
async function importLegacyMain(adapter, data) {
  ...
  for (const [alias, model] of Object.entries(data.modelAliases || {})) {
    if (isPostgres) {
      await adapter.run(`INSERT INTO kv(scope, key, value) VALUES($1, $2, $3) ON CONFLICT(scope, key) DO UPDATE SET value = excluded.value`, ['modelAliases', alias, stringifyJson(model)]);
    } else {
  ...
  for (const m of data.customModels || []) {
    ...
    if (isPostgres) {
      await adapter.run(`INSERT INTO kv(scope, key, value) VALUES($1, $2, $3) ON CONFLICT(scope, key) DO UPDATE SET value = excluded.value`, ['customModels', k, stringifyJson(m)]);
    } else {
  ...
  for (const [tool, mappings] of Object.entries(data.routerAlias || {})) {
    if (isPostgres) {
      await adapter.run(`INSERT INTO kv(scope, key, value) VALUES($1, $2, $3) ON CONFLICT(scope, key) DO UPDATE SET value = excluded.value`, ['routerAlias', tool, stringifyJson(mappings || {})]);
    } else {
      await adapter.run(`INSERT OR REPLACE INTO kv(scope, key, value) VALUES('routerAlias', ?, ?)`, [tool, stringifyJson(mappings || {})]);
    }
  }
  for (const [provider, models] of Object.entries(data.pricing || {})) {
    if (isPostgres) {
      await adapter.run(`INSERT INTO kv(scope, key, value) VALUES($1, $2, $3) ON CONFLICT(scope, key) DO UPDATE SET value = excluded.value`, ['pricing', provider, stringifyJson(models || {})]);
    } else {
...
async function importLegacyUsage(adapter, data) {
  ...
  if (typeof data.totalRequestsLifetime === "number") {
    await setMetaFromAdapter(adapter, "totalRequestsLifetime", data.totalRequestsLifetime);
  }
}
...
export async function runMigrationOnce(adapter) {
  ...
  if (fresh && hasLegacy && !alreadyImported) {
    ...
    try {
      await adapter.transaction(async () => {
        ...
        await setMetaFromAdapter(adapter, "appVersion", getAppVersion());
        await setMetaFromAdapter(adapter, "migratedAt", new Date().toISOString());
      });
    } catch (err) {
  ...
  const oldVer = await getMetaFromAdapter(adapter, "appVersion", null);
  ...
  if (oldVer && oldVer !== newVer) {
    ...
    await setMetaFromAdapter(adapter, "appVersion", newVer);
    pruneOldBackups();
