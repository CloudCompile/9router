...
// Check if settings has Fusion customModels
const hasFusionConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Fusion"));
};
...
// POST - Update Fusion customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Fusion configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Fusion"));
...
// DELETE - Remove Fusion customModels only (keep other settings)
