...
const hasFusionConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Fusion");
};

const getFusionEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Fusion") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getFusionEntry(config);
...
// POST - Apply Fusion config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Fusion entry or append
    const idx = config.findIndex((e) => e.name === "Fusion");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Fusion entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Fusion");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Fusion removed from Copilot config",
    });
  } catch (error) {
